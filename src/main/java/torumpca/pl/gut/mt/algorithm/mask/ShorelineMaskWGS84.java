...
public class ShorelineMaskWGS84 implements Mask {
    ...
    public ShorelineMaskWGS84() {
        ...
        try {
            ...
            LOG.error("failed to load shoreline data fron {} resource file", SHORELINE_FILE_NAME,
                    e);
        }
    }
    ...
    private LineString getPathRepresentation(Coordinates source, Coordinates target) {
        String wkt = String.format(PATH_PATTERN, source.latitude, source.longitude, target.latitude,
                target.longitude);
        try {
